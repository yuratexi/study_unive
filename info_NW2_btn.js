  ...
  function main(){
    ...
    switch(n){  //q=問題,a1=答え(1つめ),a2=答え(2つめ)...,k=答えの数
        ...
        case 2:
          m = getrandom(1, 20);
          switch (m) {
              case 1:q=`SMTPの特徴として正しいものはどれか？?`;a=`メールの送信時に使用され、通常TCPのポート25を使用する`;s1=`メールをサーバからダウンロードしてクライアントで管理するプロトコル`;s2=`サーバ上でメールを管理し、どこからでもアクセス可能にする`;s3=`メールの内容を暗号化するための拡張プロトコル`;atxt=``;break;
              case 2:q=`POP3の特徴として最も適切なものはどれか？?`;a=`メールを一度ダウンロードすると、通常はサーバから削除される`;s1=`メールは常にサーバ上に保管される`;s2=`送信時に使われるプロトコルである`;s3=`メールクライアントとサーバ間の暗号化が必須である`;atxt=``;break;
              case 3:q=`IMAP4を使用するメリットとして最も適切なものはどれか？?`;a=`複数端末から同じメールにアクセスできる`;s1=`メールソフトが不要になる`;s2=`すべてのメールがローカルでしか管理できない`;s3=`メールの送信が高速になる`;atxt=``;break;
              case 4:q=`電子メールのセキュリティに関する説明として誤っているものはどれか？`;a=`POP3は暗号化通信を標準で必ず行う`;s1=`SMTPは認証がないため、なりすましが可能`;s2=`S/MIMEはメール本体の暗号化に用いられる`;s3=`SSL/TLSはクライアントとサーバ間の通信を暗号化する`;atxt=``;break;
              case 5:q=`Web（WWW）の説明として最も適切なものはどれか？`;a=`インターネット上でハイパーテキストを用いて情報をやり取りする仕組み`;s1=`インターネットのハードウェアを制御するための仕組み`;s2=`クラウドストレージの一種で、ファイル共有を行う仕組み`;s3=`IPアドレスの割り当てを管理するシステム`;atxt=``;break;
              case 6:q=`HTTPに関する説明として正しいものはどれか？`;a=`クライアントのリクエストに対して、サーバがレスポンスを返す`;s1=`メールの送受信のためのプロトコルである`;s2=`サーバからのデータ送信を禁止する`;s3=`主にUDPプロトコルを使用する`;atxt=``;break;
              case 7:q=`HTMLで画像を表示するための記述として最も適切なのはどれか？`;a=`<img src="cat.png" alt="かわいい猫の画像">`;s1=`<html image="cat.png">猫の写真</html>`;s2=`<image src="cat.png">猫の写真</image>`;s3=`<pic link="cat.png">猫の写真</pic>`;atxt=``;break;
              case 8:q=`絶対パスの特徴として正しいものはどれか？`;a=`常に同じ場所を示すパス`;s1=`作業しているフォルダによってパスが変わる`;s2=`現在地からの相対的な道順`;s3=`HTMLでは使えない`;atxt=``;break;
              case 9:q=`Cookieに関する説明として正しいものはどれか？`;a=`ブラウザがサーバから受け取り、次回のアクセス時に送り返す情報`;s1=`Webページの見た目を装飾するための仕組み`;s2=`ブラウザのキャッシュ情報を暗号化する方式`;s3=`サーバのハードウェア構成を保存するための技術`;atxt=``;break;
              case 10:q=`HTTPS通信で使用されるプロトコルはどれか？`;a=`TLS`;s1=`FTP`;s2=`SMTP`;s3=`POP3`;atxt=``;break;
              case 11:q=`TLS 1.3 の特徴として適切なものはどれか？`;a=`低遅延で安全性の高い通信が可能`;s1=`通信開始までに4回のやり取りが必要`;s2=`古い暗号技術の互換性を重視している`;s3=`主に電子メールの送受信に使用される`;atxt=``;break;
              case 12:q=`電子署名に関する説明として正しいものはどれか？`;a=`第三者（認証局）の署名で正当性を保証する`;s1=`送信者の「公開鍵」で署名を生成する`;s2=`電子署名はパスワードでしか解除できない`;s3=`電子署名を使うとWebページの表示が早くなる`;atxt=``;break;
              case 13:q=`WSL（Windows Subsystem for Linux）に関する説明として正しいものはどれか？`;a=`Windows上でLinux環境を実行するための仕組み`;s1=`WindowsでLinuxのウィンドウを表示するためのグラフィックソフト`;s2=`LinuxのファイルをWindowsに変換するためのアプリケーション`;s3=`Windows専用のメール送受信プロトコル`;atxt=``;break;
